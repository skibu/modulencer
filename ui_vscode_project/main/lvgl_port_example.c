...
static esp_err_t app_touch_init(void) {
    ...
    if (result != ESP_OK) {
        ESP_LOGE(TAG, "I2C driver installation failed result=%d: %s", result, esp_err_to_name(result));
        ESP_LOGE(TAG, "But continuing anyway");
    }
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c((esp_lcd_i2c_bus_handle_t)EXAMPLE_TOUCH_I2C_NUM,
